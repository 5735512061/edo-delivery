        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">เลือกร้านอาหาร เพื่อตรวจสอบคำสั่งซื้อ</h4>
                        ...
                        @foreach ($stores as $store => $value)
                            @php
                                $order = DB::table('orders')
                                        ...
                                        ->whereNull('status')->where('store_id',$value->id)->groupBy('orders.bill_number')->get();
							    $order_count = count($order);
                            @endphp 
                            <a href="{{url('/admin/order')}}/{{$value->name}}-{{$value->branch}}" class="btn btn-primary btn-border">{{$value->name}} - {{$value->branch}} [ {{$order_count}} ]</a>
                        @endforeach
                    </p>
                </div>
            </div>
...
<div class="page-inner">
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <form action="{{url('/admin/search-order')}}" enctype="multipart/form-data" method="post">@csrf
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-3">
                            ...
                            <tbody>
                                @foreach ($orders as $order => $value)
                                    <tr>
                                        ...
                                        @endphp
                                        <td>{{$customer_code}}</td>
                                        <td><a href="{{url('/admin/order-detail/')}}/{{$value->id}}" style="color: blue;">{{$value->bill_number}}</a></td>
                                        <td>{{$value->date}}</td>
                                        <td>{{$qty}} รายการ</td>
                                        @php
                                        ...
                                        <td>       
                                            <a href="{{url('/admin/order-detail/')}}/{{$value->id}}" style="color: blue;">
                                                ตรวจสอบการสั่งซื้อ
                                            </a>
                                        </td>
                                        <td>
                                            @if($status == null || $status == 'รอยืนยัน')
                                                <a href="{{url('/admin/delete-order/')}}/{{$value->id}}" style="color: red;" onclick="return confirm('ต้องการยกเลิกคำสั่งซื้อ ?')">
                                                    ยกเลิกการสั่งซื้อ
                                                </a>
                                            @elseif($status == 'กำลังจัดส่ง')
