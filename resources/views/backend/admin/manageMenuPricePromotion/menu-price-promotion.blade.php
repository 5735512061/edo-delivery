        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">เลือกร้านอาหาร เพื่อดูรายการราคาอาหารโปรโมชั่น</h4>
                    
                </div>
                <div class="card-body">
                    <p class="demo">
                        @foreach ($stores as $store => $value)
                            <a href="{{url('/admin/list-menu-price-promotion')}}/{{$value->name}}-{{$value->branch}}" class="btn btn-primary btn-border">{{$value->name}} - {{$value->branch}}</a>
                        @endforeach
                    </p>
                </div>
            </div>
...
<div class="page-inner">
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <form action="{{url('/admin/search-menu-price-promotion')}}" enctype="multipart/form-data" method="post">@csrf
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-3">
                        ...
                        <table class="table table-hover">
                            ...
                            <tbody>
                                ...
                                @php
                                    ...
                                    $promotion_price = DB::table('menu_price_promotions')->where('menu_id',$value->id)->orderBy('id','desc')->value('promotion_price');
                                    $promotion_price_format = number_format($promotion_price);
                                @endphp
                                    <tr>
                                        ...
                                        @else 
                                            <td>{{$promotion_price_format}} บาท</td>
                                        @endif
                                        <td>       
                                            <a data-toggle="modal" data-target="#EditPrice{{$value->id}}">
                                                <i class="fas fa-edit" style="color:blue;"></i>
                                            </a>        
                                            <a href="{{url('/admin/menu-price-promotion-detail')}}/{{$value->id}}">
                                        ...
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="exampleModalLongTitle">แก้ไขราคาอาหารโปรโมชั่น</h5>
                                                </div>
                                                <form action="{{url('/admin/update-menu-price-promotion')}}" enctype="multipart/form-data" method="post">@csrf
                                                    ...
                                                    <div class="modal-body">
                                                        <div class="col-md-12">
                                                            <div class="form-group">
                                                                <label>ราคาอาหารโปรโมชั่นล่าสุด</label>
                                                                @if ($errors->has('promotion_price'))
                                                                    <span class="text-danger" style="font-size: 16px;">({{ $errors->first('promotion_price') }})</span>
                                                                @endif
                                                                <input type="text" class="form-control form-control-sm" name="promotion_price">
                                                            </div>
                                                            <div class="form-group">
                                                                <label>สถานะ</label>
